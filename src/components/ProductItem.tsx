...
const ProductItem: React.FC<ProductItemProps> = ({ product, onAddToCart }) => {
  ...
  return (
    <div className="product-item" style={cardStyle}>
      ...
      <h2 style={{ margin: '8px 0 4px 0', fontSize: '1.1rem', fontWeight: 600 }}>{product.name}</h2>
      <div style={{ color: '#888', fontSize: 14, marginBottom: 4 }}>{product.category}</div>
      <div style={priceStyle}>
        ${product.variants.length > 0
          ? Math.min(...product.variants.map(v => v.price)).toFixed(2)
          : product.price.toFixed(2)}
      </div>
      <Button variant="primary" onClick={() => setModalOpen(true)} style={{ minWidth: 120 }}>
        Add to Cart
      </Button>
      <ProductModal
        product={product}
